...
- Cuando tengas objetivo + al menos una fuente o deadline, incluye en la última línea
  la señal ONBOARDING_COMPLETE (el usuario NO la verá — es interna).
"""
...
async def onboarding_node(state: SynapseState) -> dict:
    ...
    if "ONBOARDING_COMPLETE" in content:
        response.content = content.replace("ONBOARDING_COMPLETE", "").strip()
